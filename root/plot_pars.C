...
void plot_pars()
{
    ...
    std::string path = main_dir()+"/analysis/COMPASS_pi1/pars/";

    // -----------------------------------------------------------------------
    // Import fit values

    auto Nc   = import_data<3>(path+"modNc.dat");
    auto Nd   = import_data<3>(path+"modNd.dat");
    auto phi  = import_data<3>(path+"argNd.dat");
