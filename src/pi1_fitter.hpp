...
namespace iterateKT { namespace COMPASS
{
    ...
    static std::vector<double> m3pi_bins(int min, int max)
    {
        std::vector<double> x;
        for (int i = min-11; i <= max-11; i++) x.push_back(m_bins[i]);
        return x;
    };
    ...
    inline std::tuple<std::vector<double>,std::vector<complex>> import_parameters(std::array<int,2> minmax, std::string in_pars_file)
    {
        std::vector<double> m3pis;
    ...
    struct fit_2D
    {
        ...
        static std::vector<complex> process_parameters(std::vector<complex> pars, amplitude to_fit)
        {
            ...
            int N = (pars.size()-2)/2;
            complex b1 = pars.end()[-2]; // Second to last par
            complex b2 = pars.end()[-1]; // Last par
            ...
            for (int i = 0; i < N; i++)
            {
                ...
                for (int j = 0; j < 4; j++)
                {
                    ...
                    complex N1 = pars[2*i]  *csqrt(t_bins[j])*exp(b1*(t_bins[j]-t0));
                    complex N2 = pars[2*i+1]*csqrt(t_bins[j])*exp(b2*(t_bins[j]-t0));
                    to_fit->set_parameters({N1, N2});
                };
            };
            return pars;
        };
